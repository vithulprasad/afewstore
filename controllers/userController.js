...
const join = async(req,res)=>{
    try {
        ...
        console.log('join jest for rendering the page ',error.message);
        res.render('500error')
    }
}
// =======================================================INSERT USER==================================================================
...
const insertUser = async(req,res)=>{
    ...
    if(checkMail)
    {
        res.render('registration',{message:"mail id already exists choose a different email"})
    }
   else{
      ...
      if(userData){
        res.render('login',{message:"your registration has been successful now you can login to ARNOZE"});
        console.log('user data') ;
        const categoryData = await categoryDb.findOne({ _id: _id })
          // users.updateOne({name:req.body.name},{$set:{is_verified:1}})   
    }
      else{
...
const homeLoad = async(req,res)=>{
    try {
      if(req.session.user_id){
        ...
        console.log(bannerData.description);
        ...
        const orders = await Order.find({user:userData,status:'OrderCanceled'});
        

console.log(orders.length,'-------------orders--------------------------------------------------------');
        res.render('index',{productData,bannerData,userData,categoryData,count,orders})
...
const home = async(req,res)=>{
    try {
        ...
        console.log("the error in the load home"+error.message);
        res.render('500error')
...
const productSingle = async(req,res)=>{
  try {
    ...
    console.log('for rendering the deferend page ',error.message);
    res.render('500error')
...
const cartAdd = async (req, res) => {
  try {
    ...
    if (userId) {
      ...
      const cartPurchase = await Cart.findOne({ $and: [
        { user: userId},
        {  active: 1 }
      ]})
      if (cartData) {
        ...
        if (productIndex !== -1) {
          ...
          const cartUpdate = await Cart.updateOne(
            { user: userId },
            { $push: { products: { productId, quantity, price , name,image} } }
          );
        }
      } else {
        const cart = new Cart({
      ...
      if (userCart) {
          ...
          if(products.length>0){
           ...
           const a =add.reduce((ctotal,items)=>{
            ...
            const value = await Cart.findOneAndUpdate({user: req.session.user_id },{$set:{totalPrice:a,coupon:0,couponNumber:0}}).then((result)=>{
         
          
              res.render('cart', {products,cartProducts,count,CartDa,a,userData})
            })
...
const checkout = async(req,res)=>{
  try {
    ...
    const one = await Cart.findOne({  $and: [
      { user: id},
      { status: 1 }
    ]})
    ...
    let check = await Cart.findOne({user:id,'products.productId':pId})
...
const checkoutAdd = async(req,res)=>{
  try {
    ...
    await data.save()
    const update = await Cart.findOneAndUpdate({user:id},{$set:{status:1}})
    res.redirect('/')
     } catch (error) {
          ...
          for (let i = 0; i < length; i++) {
                   if(value[i]._id==productId){
                      for(let j = 0; j < find1.address.length; j++) {                
                       ...
                       const values1=  await Product.findOneAndUpdate({_id:productId},{$set:{qunatity:quantity1}})
                         break;
                      }     
             }     
           const setProduct = await Product.updateOne({_id:productId},{$set:{available:1}})
           await Cart.findOneAndUpdate(
             { user: userId,
        ...
        for (let i = 0; i < order.length; i++) {
           if( length==i){
            ...
            for (let n = 0; n < find1.products.length; n++) {
              ...
              const category1 = await Product.findOne({_id: productId}).populate('category');
              ...
              // const user1=await users.findOne({_id:req.session.user._id});
              
              await Order.findOneAndUpdate({_id:f},{$set:{totalPrice:totalPrice}})
              await Order.updateOne(
                { _id: orderId },
                { $push: { products: { name, price,productId, image, quantity, category, } } },
                
               ).then((result)=>{
                console.log(result,'this iis the changed purchase ouder pushing');
              })
          ...
          await Cart.findOneAndUpdate(
           ...
           'products.productId':productId},
             ...
             await Cart.findOneAndUpdate(
              { user: userId,
             'products.productId':productId},
               { $set: { couponNumber:0} }  ).then((result) => {
               })
             for (let i = 0; i < find1.products.length; i++) {
               ...
               try {
                 ...
                 console.log(`Successfully removed product with ObjectId ${productId} from cart with ObjectId ${result._id}`);
               } catch (err) {
                 console.error(err);
               }
             }
...
for (let i = 0; i < find1.address.length; i++) {
        for(let j = 0; j < find1.products.length; j++) {
         ...
         await users.updateOne({_id:userId},{$set:{wallet:wallet-find1.totalPrice}});            
          const order = await Order.find()
          const length=order.length-1
        for (let i = 0; i < order.length; i++) {
           if( length==i){
...
const rozoPayment = async(req,res)=>{
  console.log('enterint to the rozopayment transaction');
  try {
    ...
    let generated_signature = hmac_sha256(razorpay_order_id + "|" + razorpay_payment_id, keysecret);

    if (generated_signature == razorpay_signature) {
      console.log("entered in the final order check---------1")
      const find = await Cart.findOne({user:user})
      const order = await Order.findOne().sort({date:-1})
      const id = order._id;
      console.log(id,'idees')
      ...
      for(let i = 0 ;i<find.products.length;i++){
        ...
        await Order.updateOne(
          { _id: id },
          { $push: { products: { name,price,image,quantity,productId,category } } }
        ).then((result)=>{
  ...
  try {
   ...
   for (let i = 0; i < checkIsBuyed.products.length; i++) {
        ...
        const totalQuantity = productQuantity+orderQuantity;
                     ...
                     await Product.updateOne({_id:orderIds},{$set:{quantity:totalQuantity}})
     }
  if (checkIsBuyed.status=="OrderCanceled") {res.redirect('/confirmation') }
  else{
    if(checkIsBuyed.status=="compleat"){
      res.redirect('/confirmation')
     }else{
        if(checkIsBuyed.orderType=="Razorpay"){
          await Order.findOneAndUpdate({_id:orderId},{$set:{status:"OrderCanceled"}})
          await users.findOneAndUpdate({_id:userid},{$set:{wallet:value}}).then(()=>{
            res.redirect('/confirmation')
          })
...
const returnOrder = async(req,res)=>{
  try {
    ...
    await Order.findOneAndUpdate({_id:id},{$set:{status:"returnOrder"}})
    await users.findOneAndUpdate({_id:user},{$set:{wallet:wallet}}).then((result)=>{
      log(result);
      res.redirect('/confirmation')
    })
...
const profile = async(req,res)=>{
  try {
    ...
    if(cart.address[0]!=undefined){
      const cod = await Order.find({user:id,orderType:"cashOnDelivery"}).count()
      const rozorpay = await Order.find({user:id,orderType:"Razorpay"}).count()
      const wallet = await Order.find({user:id,orderType:"walletPayment"}).count()
  ...
  console.log('data of the product ',cod,'-----',rozorpay,'------','wallet---',wallet);
      
      const userData = await users.findOne({_id:id})
      res.render('profile',{userData,cart,cod,rozorpay,wallet,serch,exp})
    }else{
      console.log('data is now avai');
...
const profileEdit = async(req,res)=>{
  try {
    ...
    if(req.files==''){
      const userID=req.session.user_id;
      const update = await users.findByIdAndUpdate({_id:userID},{ $set:{ name:req.body.name,phone:req.body.phone}}).then((value)=>{
        console.log(value);
        res.redirect('/profile')
      })
    }
    const userID=req.session.user_id;
    const update = await users.findByIdAndUpdate({_id:userID},{ $set:{ name:req.body.name,phone:req.body.phone,image:req.files[0].filename}}).then((value)=>{
      console.log(value);
      res.redirect('/profile')
    })
  } catch (error) {
...
const addressAdd = async(req,res)=>{
  ...
  if(productId){
    ...
    const user = req.session.user_id;
    await Cart.findOneAndUpdate({user:user},{$push:{address:{firstName,lastName,house,post,city,district,state,pin}}})
...
const UsedAddress = async(req,res)=>{
  try {
   const user = req.session.user_id;
   const orders = await Order.find({user:user,status:'OrderCanceled'});
   const usersData = await users.findOne({_id:user})
   res.render('wallethistory',{orders,usersData})
  } catch (error) {
    console.log(error.message);
    res.render('500error')
         ...
         'products.productId':productId},
           ...
           for(var j=0;j<CartData.address.length;j++){
            if(CartData.address[j]._id ==addressId ){
              ...
              res.json({firstName,lastName,house,post,city,district,state,pin});
            }
          }       
      }
    }
  ...
  try {
    ...
    if(user){
      ...
      console.log( userDATA.wishlist.length);
         ...
         console.log( userDATA.wishlist.length);
         if(count==0){
          await users.findOneAndUpdate({_id:user},{$push:{wishlist:{productId,Price:product.price,name:product.name,quantity:product.quantity,image:product.image[0]}}})
          const value = 1;
          res.json({value});
...
const ChangeCategory = async (req, res) => {
  console.log('data is coming to the change pafge------');
  try {
    ...
    const pipeline = [
      ...
      { $sort: { _id: -1 } }, // sort in descending order to get latest items first
      { $skip: skip }, // skip items based on current page
      { $limit: limit }, // limit items to 10 per page
      {
        $project: {
          _id: 1,
          name: 1,
          ...
          offerPrice: 1,
          quantity: 1,
          image:1, // get only first image from the array
        },
      },
    ];
 
    const productData = await Product.aggregate(pipeline);
    console.log(productData,'johsdfasdhfoisdhfaodshfo-------');
    const categoryData = await Category.find();
    const count = await Product.countDocuments({ category: catId });

    let responseData = { categoryData, productData, count, category, currentPage: currentPage, pages: Math.ceil(count / limit) };
    if (user) {
      const cartData = await Cart.findOne({ user });
      responseData = { ...responseData, cartData };
    }
    res.json({responseData,currentPage,category,productData,categoryData,count});
  } catch (error) {
    console.log(error.message);
    res.render('500error');
  }
...
if(user){
  ...
  if(catId!='empty'){
    const categoryData = await Category.find()
    const productData = await Product.findOne({ name:value,'category':catId }).populate('category')
  ...
  try {
    ...
    if(user){                                   
      ...
      if(type =='LOW'){
        if(catId!='empty'){
          const categoryData = await Category.find()
          const productData = await Product.find({ 'category':catId }).sort({price:-1}).populate('category')
        ...
        if(catId!='empty'){
          const categoryData = await Category.find()
          const productData = await Product.find({ 'category':catId }).sort({date:1}).populate('category')
    ...
    await Cart.findOneAndUpdate(
      ...
      for(let i=0;i<find.products.length;i++){
        if(find.products[i].productId == productId){
          if(find.products[i].quantity!=1||find.products[i].quantity!=0){
          ...
          console.log('quantity is less than one------------------------------------------');
        }
...
const setSingle = async (req,res)=>{
  try {
        ...
        await Product.findOneAndUpdate({_id:productId }, {$inc: {qun: operator}}).then((product)=>{console.log('sucses--------------------------');});
...
const couponApply = async(req,res)=>{
     ...
     const cartData = await Cart.findOne({user:user});
      ...
      const couponMatch = await Order.findOne({user:user,couponNumber:value})
      ...
      let key = 0;
        for (let i = 0; i < coupons.length; i++) {
          ...
          if(value==coupon){
            if(couponMatch){
              const returnCoupon5 = "this coupon is already in use please choose a different coupon"
              res.json({returnCoupon5});
            }else{
              if(cartData.totalPrice>couponP ){
                await Cart.findOneAndUpdate({user:user},{$set:{coupon:couponP,couponNumber:value}});
                key = 10;
                ...
                await Cart.findOneAndUpdate({user:user},{$set:{coupon:0,couponNumber:0}});
                 key = 10;
  ...
  if(confirmPass==""&&newPassword=="")
   {
    ...
    const value2=parseInt(confirmPass)
      if(value1==value2){
        ...
        await users.findOneAndUpdate({_id:user},{$set:{password:passwordHash}}).then((result) => {
          console.log(result);
          const value = 4;
          res.json({value})
        })  
        ...
        const value = 5;
       const message = 'password not matching please enter correct password'
        res.json({value,message})
      }
   }
  }
