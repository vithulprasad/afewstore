...
const loadHome = async(req,res)=>{
    try {
      ...
      const orderDataRecent = await Orders.find().sort({date:-1}).populate('user');
...
const productLoad = async(req,res)=>{
  console.log('the product load is working-------------------------------------------------------------------------------------');
...
const addProductLoad = async(req,res)=>{
    try {
      console.log('the addd product load is working-------------------------------------------------------------------------------------');
        const  category = await categoryDb.find({});
        const message = 'true';
...
if(req.body.offerPrice  <=  req.body.price){
  ...
  const product = await Product.find({})
    const category = await categoryDb.find({})
  const message = 'the product price is smaller compared to to offer please chage';
  res.render('productAdd',{product,category,message})
}
...
const updateProduct = async(req,res)=>{
  try {
     const user_id = req.query.id
      const productData = await Product.findOne({ _id: user_id }).populate('category');
    const  category = await categoryDb.find({});
    const message= 'true'
  ...
  const productUpdate = async(req,res)=>{
    try {
     ...
     await Product.findByIdAndUpdate({ _id: product_id }, { $set: { name: req.body.name,quantity:req.body.que ,discription:req.body.discription,price:req.body.price,category:req.body.category,offerPrice:req.body.offerPrice} }).then((value)=>{
      res.redirect('/admin/product');
     })
      ...
      if(check){
        ...
        for (let i=0;i<find.length;i++){
          if(find[i].category._id==product_id){
           ...
           const Data = await Product.updateMany({ 'category.$._id': value}, { $set: {  name: req.body.discription,images:images[0].filename } })
          }
     }
       const Data = await categoryDb.findByIdAndUpdate({ _id: product_id }, { $set: {  name: req.body.discription,images:images[0].filename } })
     res.redirect('/admin/category')
      }
...
const banned = async(req,res)=>{
        try {
          const id = req.query.id;
           const check = await users.updateOne({ _id:id},{$set:{status:"banned"}});
...
const unbanned = async(req,res)=>{
  try {
    ...
    const check = await users.updateOne({ _id:id},{$set:{status:"unbanned"}});
     const productData = await users.find({ is_verified:0});
...
const OrderShipped = async(req,res)=>{
  try {
    ...
    if(find.status=='complete'||find.status=='OrderCanceled'){
      ...
      await Orders.findOneAndUpdate({_id:id},{$set:{status:"OrderShipped"}})
      res.redirect('/admin/order');
    }
...
const Deliverd= async(req,res)=>{
  try {
    ...
    if(find.status=='complete'||find.status=='OrderCanceled'){
      ...
      await Orders.findOneAndUpdate({_id:id},{$set:{status:"Deliverd"}})
      res.redirect('/admin/order');
    }
...
const OrderCanceled = async(req,res)=>{
  try {
    ...
    if(find.status=='complete'){
      ...
      await Orders.findOneAndUpdate({_id:id},{$set:{status:"OrderCanceled"}})
      res.redirect('/admin/order');
    }
...
const compleat = async(req,res)=>{
  try {
    console.log('---------------------orderd--------------------');
    const id = req.query.id;
    await Orders.findOneAndUpdate({_id:id},{$set:{status:"compleat"}})
    res.redirect('/admin/order');
  } catch (error) {
    console.log(error.message);
  }
